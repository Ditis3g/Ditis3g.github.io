...
#   3) D:\blog-memo 폴더 생성  4) 매일 23:00 작업 스케줄러 등록
#   5) blog-memo 스킬 설치
...
Register-ScheduledTask -TaskName 'DailyBlogDraft' -Action $action -Trigger $trigger -Settings $settings `
  -Principal $principal -Description '매일 밤 11시 D:\blog-memo 메모를 블로그 초안으로 정리해 올림' -Force | Out-Null
Write-Host "[OK] 작업 스케줄러 등록됨 (매일 23:00)"

# 6) blog-memo 스킬 설치 (저장소 -> 개인 스킬 폴더)
$skillSrc = Join-Path $BlogDir 'skills\blog-memo'
$skillDst = Join-Path $env:USERPROFILE '.claude\skills\blog-memo'
if (Test-Path $skillSrc) {
  New-Item -ItemType Directory -Force -Path (Split-Path $skillDst -Parent) | Out-Null
  Copy-Item $skillSrc $skillDst -Recurse -Force
  Write-Host "[OK] blog-memo 스킬 설치됨 ($skillDst)"
} else {
  Write-Host "[!] skills\blog-memo 가 없어 스킬 설치를 건너뜀"
}
...
Write-Host "  - 테스트:  D:\blog-memo 에 메모 .txt 하나 넣고 아래 실행:"
Write-Host "    powershell -ExecutionPolicy Bypass -File `"$scriptPath`""
Write-Host "  - blog-memo 스킬은 Claude Code를 재시작해야 인식됩니다."
